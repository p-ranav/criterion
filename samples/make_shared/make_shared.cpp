...
BENCHMARK(ConstructSharedPtr, std::function<std::shared_ptr<Foo>()>) {
  SETUP_BENCHMARK(auto test_function = GET_ARGUMENT(0);)

  auto foo_ptr = test_function();

  TEARDOWN_BENCHMARK(foo_ptr.reset();)
}
...
REGISTER_BENCHMARK_FOR_EACH(ConstructSharedPtr, ("/new", Create_Foo_With_New),
